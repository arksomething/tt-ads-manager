...
export interface CreatorTrackerIngestionKey {
  id: string;
  secret: Buffer;
}

export interface CreatorTrackerIngestionRuntimeConfig {
  databaseUrl: string;
  databaseTls: {
    ca: string;
    servername: string;
...
function parseDatabaseUrl(value: string | undefined) {
  ...
  // object. Remove the reviewed marker and pass a pinned CA configuration to
  // Pool instead; no URL parameter is allowed to weaken it.
...
export function loadCreatorTrackerIngestionRuntimeConfig(
  ...
  if (
    ...
    organizationIds.some(
      (entry) =>
        entry.length > 256 ||
        !NO_CONTROL_OR_WHITESPACE.test(entry),
    )
  ) {
    throw new CreatorTrackerIngestionConfigurationError();
  }
...
const NO_CONTROL_OR_WHITESPACE = /^[^\u0000-\u0020\u007f]+$/;
...
export function authenticateCreatorTrackerIngestionRequest(
  ...
  if (contentEncoding && contentEncoding.toLowerCase() !== "identity") {
    throw new CreatorTrackerIngestionRequestError(415, "UNSUPPORTED_CONTENT_ENCODING");
  }

  const keyId = request.headers.get("x-creator-ingest-key-id") ?? "";
  const timestamp = request.headers.get("x-creator-ingest-timestamp") ?? "";
  const claimedHash =
    request.headers.get("x-creator-ingest-content-sha256") ?? "";
  const suppliedSignature =
    request.headers.get("x-creator-ingest-signature") ?? "";

  if (
    !KEY_ID_PATTERN.test(keyId) ||
    !/^\d{10}$/.test(timestamp) ||
    !SHA256_PATTERN.test(claimedHash) ||
  ...
  const verificationSecret = selectedKey?.secret ?? Buffer.alloc(MIN_KEY_BYTES);
