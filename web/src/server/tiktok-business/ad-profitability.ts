...
function normalizeResolvedPost(
  ...
  const createTime = getFirstString(candidates, [
    "create_time",
    "createTime",
    "publish_time",
    "publishTime",
  ...
  if (!title && !coverUrl && !shareUrl && !createTime) {
    return null;
  }
    ...
    warnings: uniqueNonEmptyStrings([
      ...
      ...(resolvedPostsByItemId.size === 0
        ...
        : resolvedPostsByItemId.size < candidateItemIds.length
          ? [
              `Resolved exact public post info for ${resolvedPostsByItemId.size} of ${candidateItemIds.length} candidate TikTok post IDs relevant to this request. TikTok hid the rest, so the remaining matches use ad names, Ads Manager ad links, and Singular metadata when available.`,
            ]
          : []),
    ]),
  };
