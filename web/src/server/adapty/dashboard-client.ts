...
function getInternalIds(rows: readonly unknown[]) {
  return rows
    .map((row) =>
      isRecord(row) &&
      (typeof row.internal_id === "string" || typeof row.internal_id === "number")
...
export async function getAppleSearchAdsDashboardReport(args: {
  ...
  const dbCredentials = args.organizationSlug
    ? await import("../settings/managed-secrets.ts").then((module) =>
        module.getAdaptyDashboardCredentials(args.organizationSlug ?? ""),
      )
    : null;
  const credentials = dbCredentials?.configured ? dbCredentials.value : undefined;

  if (!credentials && !hasAdaptyDashboardEnv()) {
    return {
      configured: false,
      conversions: null,
      installs: null,
