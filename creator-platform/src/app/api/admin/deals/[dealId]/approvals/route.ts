...
import { parseAdminDealApprovalInput } from "@/server/admin/deals";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const parsed = parseAdminDealApprovalInput(await parseDealJsonBody(request));
  if (!parsed) {
    return dealJson(authResponse, {
      error: "Provide an approval kind, the exact sealed snapshot hash, and a legal review reference for legal approval.",
    }, 400);
  }

  const session = await requireAdminDealSession(request, authResponse);
  if (session instanceof NextResponse) return session;
  const { data, error } = await session.supabase.rpc("record_admin_program_deal_approval", {
    target_deal_version_id: dealId,
    approval_kind_input: parsed.kind,
    expected_snapshot_sha256: parsed.snapshotHash,
    note_input: parsed.note,
  });
