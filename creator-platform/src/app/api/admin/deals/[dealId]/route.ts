...
import { parseAdminDealUpdateInput } from "@/server/admin/deals";
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const parsed = parseAdminDealUpdateInput(await parseDealJsonBody(request));
  if (!parsed) {
    return dealJson(authResponse, {
      error: "Provide the current draft revision and at least one valid content change.",
    }, 400);
  }
  ...
  const { data, error } = await session.supabase.rpc("update_admin_program_deal_draft", {
    target_deal_version_id: dealId,
    expected_draft_revision: parsed.revision,
    draft_patch: parsed.patch,
  });
