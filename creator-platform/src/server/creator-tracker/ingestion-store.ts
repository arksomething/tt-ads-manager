...
type CreatorTrackerVideo = CreatorTrackerIngestionBatch["videos"][number];
type CreatorTrackerObservation =
  CreatorTrackerIngestionBatch["observations"][number];

export interface ExistingCreatorTrackerVideoFirstSeen {
  id: string;
  firstSeenAt: string;
  firstSeenRunId: string;
  captionFirst: string | null;
  hashtagsFirst: string[];
}

export function resolveCreatorTrackerVideoFirstSeenRows(
  videos: readonly CreatorTrackerVideo[],
  existingVideos: readonly ExistingCreatorTrackerVideoFirstSeen[],
  availableRunIds: ReadonlySet<string>,
  batchId: string,
) {
  const existingById = new Map(
    existingVideos.map((video) => [video.id, video] as const),
  );
  if (existingById.size !== existingVideos.length) {
    throw new CreatorTrackerIngestionStoreError(
      "internal",
      "DUPLICATE_EXISTING_VIDEO",
    );
  }

  return videos.map((video) => {
    const existing = existingById.get(video.id);
    let firstSeenRunId: string;
    let resolution:
      | "claimed_run"
      | "canonical_evidence_fallback"
      | "existing_canonical_authority";

    if (existing) {
      firstSeenRunId = existing.firstSeenRunId;
      resolution = firstSeenRunId === video.firstSeenRunId
        ? "claimed_run"
        : "existing_canonical_authority";
    } else if (availableRunIds.has(video.firstSeenRunId)) {
      firstSeenRunId = video.firstSeenRunId;
      resolution = "claimed_run";
    } else if (availableRunIds.has(video.evidenceRunId)) {
      // A laptop-local first-seen run can predate canonical delivery. In that
      // case the first durable canonical evidence is this signed page run.
      // Preserve the collector's historical claim in metadata while keeping
      // the normalized FK attached to evidence that was actually committed.
      firstSeenRunId = video.evidenceRunId;
      resolution = "canonical_evidence_fallback";
    } else {
      throw new CreatorTrackerIngestionStoreError(
        "invalid_evidence",
        "FIRST_SEEN_RUN_UNRESOLVED",
      );
    }

    const metadata = withIngestionLineage(
      video.metadata,
      batchId,
      video.evidenceRunId,
      video.rawManifestId,
    );
    const lineage = metadata._creatorTrackerIngestion;
    return {
      ...video,
      firstSeenAt: existing?.firstSeenAt ?? video.firstSeenAt,
      firstSeenRunId,
      captionFirst: existing ? existing.captionFirst : video.captionFirst,
      hashtagsFirst: existing ? [...existing.hashtagsFirst] : video.hashtagsFirst,
      metadata: {
        ...metadata,
        _creatorTrackerIngestion: {
          ...lineage,
          claimedFirstSeenRunId: video.firstSeenRunId,
          firstSeenRunResolution: resolution,
          historicalIdentityResolution: existing
            ? "existing_canonical_authority"
            : "incoming_canonical_claim",
        },
      },
    };
  });
...
export interface CreatorTrackerDerivedRegression {
  id: string;
  computedRegression: boolean;
}

export function applyCreatorTrackerDerivedRegressions(
  observations: readonly CreatorTrackerObservation[],
  derived: readonly CreatorTrackerDerivedRegression[],
) {
  const byId = new Map(derived.map((entry) => [entry.id, entry] as const));
  if (byId.size !== derived.length || byId.size !== observations.length) {
    throw new CreatorTrackerIngestionStoreError(
      "internal",
      "COUNTER_REGRESSION_DERIVATION_INCOMPLETE",
    );
  }
  return observations.map((observation) => {
    const resolution = byId.get(observation.id);
    if (!resolution) {
      throw new CreatorTrackerIngestionStoreError(
        "internal",
        "COUNTER_REGRESSION_DERIVATION_INCOMPLETE",
    ...
    return {
      ...observation,
      counterRegression: resolution.computedRegression,
    };
  });
}
  ...
  try {
    ...
    if (payload.videos.length > 0) {
      const existingVideoResult = await client.query<{
        ...
        caption_first: string | null;
        hashtags_first: string[];
      }>(
        `SELECT id::text, first_seen_at, first_seen_run_id::text,
                caption_first, hashtags_first
           FROM creator_tracker_v2.videos
          WHERE organization_id = $1 AND id = ANY($2::uuid[])`,
        [organizationId, payload.videos.map((entry) => entry.id)],
      );
      const referencedRunIds = [
        ...new Set(
          payload.videos.flatMap((entry) => [
            entry.firstSeenRunId,
            entry.evidenceRunId,
          ]),
        ),
      ];
      const availableRunResult = await client.query<{ id: string }>(
        `SELECT id::text
           FROM creator_tracker_v2.tracking_runs
          WHERE organization_id = $1 AND id = ANY($2::uuid[])`,
        [organizationId, referencedRunIds],
      );
      const rows = resolveCreatorTrackerVideoFirstSeenRows(
        payload.videos,
        existingVideoResult.rows.map((entry) => ({
          id: entry.id,
          firstSeenAt: timestampString(entry.first_seen_at),
          firstSeenRunId: entry.first_seen_run_id,
          captionFirst: entry.caption_first,
          hashtagsFirst: entry.hashtags_first,
        })),
        new Set(availableRunResult.rows.map((entry) => entry.id)),
        payload.batch.id,
      );
      const result = await client.query(
    ...
    if (payload.observations.length > 0) {
      const regressionResolution = await client.query<{
        id: string;
        computed_regression: boolean;
      }>(
        `WITH incoming AS (
           SELECT observation.*
             FROM jsonb_to_recordset($2::jsonb) AS observation(
               id uuid, "videoId" uuid, "observedAt" timestamptz,
               views bigint, likes bigint, comments bigint, shares bigint, saves bigint,
               availability creator_tracker_v2.video_availability,
               "isComplete" boolean, "counterRegression" boolean
             )
         )
         SELECT current_observation.id::text,
           ...
           FROM incoming AS current_observation
           LEFT JOIN LATERAL (
             SELECT candidate.views, candidate.likes, candidate.comments,
                    candidate.shares, candidate.saves
               FROM (
                 SELECT stored.views, stored.likes, stored.comments,
                        stored.shares, stored.saves, stored.observed_at, stored.id
                   FROM creator_tracker_v2.video_observations AS stored
                  WHERE stored.organization_id = $1
                    AND stored.video_id = current_observation."videoId"
                    AND stored.is_complete
                    AND stored.availability = 'available'
                    AND (stored.observed_at, stored.id)
                        < (current_observation."observedAt", current_observation.id)
                 UNION ALL
                 SELECT prior.views, prior.likes, prior.comments,
                        prior.shares, prior.saves,
                        prior."observedAt" AS observed_at, prior.id
                   FROM incoming AS prior
                  WHERE prior."videoId" = current_observation."videoId"
                    AND prior."isComplete"
                    AND prior.availability = 'available'
                    AND (prior."observedAt", prior.id)
                        < (current_observation."observedAt", current_observation.id)
               ) AS candidate
      ...
      // counterRegression is a central ordering derivative, not source
      // evidence. The exact signed collector claim remains in the staging
      // projection; normalize the queryable flag to the canonical timeline.
      const rows = applyCreatorTrackerDerivedRegressions(
        payload.observations,
        regressionResolution.rows.map((entry) => ({
          id: entry.id,
          computedRegression: entry.computed_regression,
        })),
      );
      const result = await client.query(
